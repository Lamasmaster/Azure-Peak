...
/datum/antagonist/aspirant/can_be_owned(datum/mind/new_owner)
	. = ..()
	if(.)
		if(!((new_owner.assigned_role in GLOB.noble_positions) || (new_owner.assigned_role in GLOB.retinue_positions) || (new_owner.assigned_role in GLOB.garrison_positions) || (new_owner.assigned_role in GLOB.courtier_positions)))
			return FALSE
